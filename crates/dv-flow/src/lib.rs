...
const CODE_TINT: Color = Color {
    r: 0xF2,
    g: 0xF3,
    b: 0xF5,
    a: 255,
};
const QUOTE_BAR: Color = Color {
    r: 0xC8,
    g: 0xCC,
    b: 0xD2,
    a: 255,
};
const QUOTE_TEXT: Color = Color {
    r: 0x55,
    g: 0x59,
    b: 0x60,
    a: 255,
};
const RULE_COLOR: Color = Color {
    r: 0xD0,
    g: 0xD3,
    b: 0xD8,
    a: 255,
...
impl FlowDoc {
    ...
    pub fn render_page(&self, idx: usize, scale: f32) -> DisplayList {
        ...
        let mut i = 0;
        let mut on_page: Vec<&G> = self
            .glyphs
            .iter()
            .filter(|g| g.baseline >= top - 4.0 && g.baseline <= bot + 4.0)
            .collect();
        on_page.sort_by(|a, b| {
            (
                a.size as i32,
                ...
                if !t.is_space || x > left {
                    ...
                    if t.underline {
                        this.rects.push(Rect {
                            x,
                            y: baseline + t.size * 0.12,
                            w: t.advance,
                            h: (t.size * 0.06).max(1.0),
                            color: t.color,
                        });
                    }
                    if t.strike {
